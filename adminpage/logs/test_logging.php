<?php
/**
 * Test script for Receipt Logging System
...
echo "<h2>Testing Receipt Logging System</h2>";
...
try {
    ...
    if ($result['success']) {
        ...
        echo "<p>Receipt ID: <strong>" . $result['receipt_id'] . "</strong></p>";
        echo "<p>Log ID: <strong>" . $result['log_id'] . "</strong></p>";
    } else {
        echo "<p style='color: red;'>✗ Test logging failed: " . $result['error'] . "</p>";
    }
    
    // Test retrieving logs
    $logs = $logger->getReceiptLogs(['limit' => 5]);
    echo "<p style='color: green;'>✓ Retrieved " . count($logs) . " recent logs</p>";
    
    // Test statistics
    $stats = $logger->getReceiptStats();
    echo "<p style='color: green;'>✓ Retrieved receipt statistics</p>";
    
    echo "<h3>Test Summary</h3>";
    echo "<p style='color: green; font-weight: bold;'>✓ All tests passed successfully!</p>";
